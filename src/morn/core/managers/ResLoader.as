...
package morn.core.managers {
	...
	import morn.core.utils.BMPDecoder;
	...
	public class ResLoader {
		...
		public static const MOVIE:uint = 6;
		...
		private var decoder:BMPDecoder = null;
		...
		private function doLoad():void {
			...
			if (_type == BMD || _type == AMF || _type == DB || _type == BYTE || _type == SWF || _type == MOVIE)
			{
		...
		private function onComplete(e:Event):void {
			var content:* = null;
			if (_type == SWF || _type == BMD||_type == MOVIE) 
			{
				if (_urlLoader.data != null)
				{
					if (_url.substr( -4) == ".bmp")
					{
						var myBa : ByteArray = ByteArray(_urlLoader.data);
						decoder = decoder || new BMPDecoder();
						var bmd:BitmapData=decoder.decode(myBa);
						endLoad(bmd);
						if (_isCache) {
							_loadedMap[_url] = bmd;
						}
				...
				{
					content = MovieClip(_loader.content);
					_loader.unloadAndStop();
				}
				else {
