...
package morn.core.components {
	...
	[Event(name = "imageFailed", type = "morn.core.events.UIEvent")]
	...
	public class Image extends Component {
			...
			if (_url != value) {
				...
				if (Boolean(value)) {
					if (App.asset.hasClass(_url)) {
						...
						App.mloader.loadBMD(_url, 1, new Handler(setBitmapData, [_url]),null,new Handler(loadFailed));
					}
				} else {
					bitmapData = null;
				}
		...
		protected function loadFailed(url:String):void
		{
			if (hasEventListener(UIEvent.IMAGE_FAILED))
			{
				sendEvent(UIEvent.IMAGE_FAILED);
			}
		}
